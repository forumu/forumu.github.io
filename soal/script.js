...
async function initQuiz() {
    ...
    // Load default package (paket1)
    quizData = await loadCSVFile('paket1.csv');
...
async function loadPackageHandler() {
    ...
    const newQuizData = await loadCSVFile(`${selectedPackage}.csv`);
    
    if (newQuizData && newQuizData.length > 0) {
        quizData = newQuizData;
        initQuiz();
        if (packageModal) packageModal.style.display = 'none';
        alert(`Paket ${selectedPackage} berhasil dimuat!`);
    }
}
...
function initEventListeners() {
    ...
    if (loadPackage) loadPackage.addEventListener('click', loadPackageHandler);
